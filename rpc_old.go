...
func (server *Server) Register(rcvr interface{}) os.Error {
        ...
        if _, present := server.serviceMap[sname]; present {
                return os.NewError("rpc service is already defined: " + sname)
        }
        s.name = sname
        ...
        if len(s.methods) == 0 {
                return os.NewError("register type " + sname + " has no exported methods")
        }
...
func (server *Server) CallMethod(serviceName, methodName string, args interface{}) (interface{}, os.Error) {
        srvc, present := server.serviceMap[serviceName]
        if present == false {
                return nil, os.NewError("no service found: " + serviceName)
        }
        method, present := srvc.methods[methodName]
        if present == false {
                return nil, os.NewError("no method in " + serviceName + " found named " + methodName)
        }
...
func (server *Server) replySuccess(conn net.Conn, msgid uint32, result interface{}) os.Error {
...
func (server *Server) replyError(conn net.Conn, msgid uint32, errmsg string) os.Error {
        response := make([]interface{}, 4)
        response[0] = rpc_response
        response[1] = msgid
        response[2] = errmsg
        ...
        for {
                ...
                if args[0] == rpc_request {
                        ...
                        if err != nil {
                                ...
                                server.replyError(conn, msgid, "error calling method: "+err.String())
                                continue
                        }
                        ...
                        msecs := (float64)(endTime-startTime) / 1000000
...
func (c *Client) Connect() os.Error {
...
func (c *Client) Call(method string, args interface{}) (interface{}, os.Error) {
